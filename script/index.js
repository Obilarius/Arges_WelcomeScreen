...
$(document).ready(function () {
  ...
  $("#registerModalCenter").on("show.bs.modal", function (event) {
    ...
    $("#registerModalCenter .modal-body .inputCompany").removeClass(
      "is-invalid"
    );
    ...
    if (button[0].className == "userRegisterLink") {
      ...
      modal
        .find(".modal-body .inputCompany")
        .val(company)
        .attr("readonly", true);
      if (host) {
        modal
          .find(".modal-body .inputAppointment")
          .val(host)
          .attr("readonly", true);
      } else {
        modal
      ...
      modal.find(".modal-body").data({
        appID: button.data("appid"),
        location: button.data("location"),
        host: button.data("host"),
      });
    } else {
      // Update the modal's content.
      let modal = $(this);
      ...
      if (name == "" || company == "") {
        ...
        if (host == "debug") {
          ...
          $.post(
            ...
            function (id, status) {
              if (id == -1) {
                $("#registerModalCenter .modal-footer span").css(
                  "display",
                  "inline"
                );
                ...
                if ($(".modal-body .inputAppointment").val()) {
                  var h = encodeURIComponent(
                    $(".modal-body .inputAppointment").val()
                  );
                  url += "&host=" + h;
                }
                window.open(
                  url,
                  "winPrint",
                  "width=210,height=80,resizeable=0,scrollable=0,menubar=0,status=0,top=0"
                );
                $("#registerModalCenter").modal("hide");
              }
            }
  ...
  $("#btnRegisterCheckOut").on("click", function (event) {
    ...
    $.post(
      ...
      function (data) {
        ...
        if (data == 1) {
          $(".checkout-body .checkout-msg")
            .html("You have been successfully logged out")
            .show();
        } else {
          $(".checkout-body .checkout-msg")
            .html("You are already logged out")
            .show();
        }
      }
    );
...
function dynAppointment() {
  ...
  $.post(
    ...
    {
      url:
        "//filer/Public/Datenbankentwicklung/ARGESVisitors/ARGESVisitorsList.xml",
    },
    function (data, status) {
      ...
      var jetzt = new Date(new Date().toString().split("GMT")[0] + " UTC")
        .toISOString()
        .split(".")[0];
      ...
      $.each(jsonObj, function (key, termin) {
        ...
        if (
          (getUrlParameter("show") == "all" && !getUrlParameter("date")) ||
          (getUrlParameter("show") == "all" &&
            termin_from <= previewDate &&
            previewDate <= termin_to) ||
          (termin_from <= jetzt &&
            jetzt <= termin_to &&
          ...
          terminTemplate +=
            termin._Start_Time == "00:00" ? "08:00" : termin._Start_Time;
          ...
          names.forEach(function (name) {
            ...
            let host =
              typeof termin._Host === "undefined" ? null : termin._Host.trim();

            terminTemplate +=
              '<li><span class="userRegisterLink" data-toggle="modal" data-target="#registerModalCenter"';
          ...
          let _company =
            typeof termin._Company === "undefined"
              ? ""
              : termin._Company.trim();
          terminTemplate +=
            '</ul></td><td class="col col3">' +
            _company +
            '</td><td class="col col4"><img src="' +
            termin._WelcomeLogo +
            '" alt=""></td></tr>';
...
function getCurrentInHouse() {
  $.post(
    ...
    function (data, status) {
      ...
      var tbl =
        "<table class='tbl_curret_in_house'><thead><tr><th>Name</th><th>Company</th><th>Host</th></tr></thead>";
      $.each(data, function (key, val) {
        var n = val["name"] != null ? val["name"] : "";
        var c = val["company"] != null ? val["company"] : "";
        var h = val["Host"] != null ? val["Host"] : "";
        tbl +=
          "<tr><td>" + n + "</td><td>" + c + "</td><td>" + h + "</td></tr>";
      });
      tbl += "</table>";
